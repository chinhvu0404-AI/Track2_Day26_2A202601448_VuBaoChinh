...
| `detect_enforcement_failure` | Dedicated mechanical detector for mutations that fired and were not denied. |
| 16 named `_hook_*` detectors | Conservative trace/answer checks for every remaining class, including positive/near-miss evidence handling. |
...
`enforced` pair together) is reused throughout. The deterministic classes rely on authoritative L1
events. The 8 adjudicated classes (`ungrounded`, `hallucination`, `guardrail_breach`,
`unflagged_conflict`, `incoherent`, `overreach`, `non_responsive`, `unsupported_precision`) use
deliberately conservative semantic predicates so a plausible-looking near miss does not become an
expensive false accusation.

## Developing your own detector

1. Extend the narrowest relevant hook in `_HOOKS`; keep the returned shape
   `[(evidence_refs, argument), ...]`.
2. Add both a positive and a near-miss/generalization test.
3. Rerun `score_prosecutor`; never trade recall for a nonzero false-claim rate without a measured
   reason.

```bash
python -m eval.prosecute            # scores this implementation against the labelled fixtures
...
The current implementation scores the committed labelled fixture set at:

```
precision: 1.000   recall: 1.000   f1: 1.000   false_claim_rate: 0.000
```

Treat those numbers as regression evidence for this finite fixture set, not a guarantee on unseen
live traces. The safe direction for any future change is to retain zero false claims while adding
general, evidence-backed cases.
